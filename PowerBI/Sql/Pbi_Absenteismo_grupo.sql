...
select decode(a.EmpFil, Null, b.EmpFil, a.EmpFil) EmpFil, 'X' Empresa
  ...
  From (select count(fu.nomefunc) qtd_ocorr
             ...
             , Decode(g.descgrupo, null, fu.descarea, decode(g.descgrupo,'FISCALIZAÇÃO','FISCAL RODOVIARIO', g.descgrupo)) Grupo
         ...
         group by Last_day(trunc(t.dthist))
             , fu.situacaofunc
             , fu.CODAREA, fu.DESCAREA
             , Decode(g.descgrupo, null, fu.descarea, Decode(g.descgrupo,'FISCALIZAÇÃO','FISCAL RODOVIARIO', g.descgrupo))
        ...
        select count(fu.nomefunc) qtd_ocorr
          ...
          from flp_historico t, frq_ocorrencia f, PBI_vwFuncionarios fu
         where f.codocorr = t.codocorr
           and fu.Codigoempresa || fu.Codigofl In (11,12,21,41,51,61,92,131,261,263,291)
           And ((Trunc(t.dthist) BETWEEN ADD_MONTHS(Trunc(SYSDATE,'rr'), -24) and sysdate
           and f.codocorr in(505,504,506,507,521,526,534,535,541,542,579,582,579,560,584,580,581,583,592,593,527,601,602,608,609))
            Or (Trunc(t.dthist) BETWEEN ADD_MONTHS(Trunc(SYSDATE,'rr'), -24) and Sysdate
           And fu.CODAREA = 40
       ...
       (Select max(Total_func) Total_func, Data, CodArea, EmpFil, Grupo, DescArea
          ...
          From (Select Empresa, Empfil, Max(Total_Func) Total_Func, Data, CodArea, descArea, grupo
           From (select 'X' empresa
                   ...
                   from PBI_vwFuncionarios f, flp_fichafinanceira ff, Flp_Funcionarios_Grupo fg, Flp_Grupo g
                  ...
                  , CodArea, descArea, Decode(g.descgrupo, null, f.descarea, Decode(g.descgrupo,'FISCALIZAÇÃO','FISCAL RODOVIARIO', g.descgrupo))
                ...
                Select max(Total_func) Total_func, Data, CodArea, descArea, EmpFil, descArea Grupo
                  From (Select Empresa, Empfil, Max(Total_Func) Total_Func, Data, CodArea, descArea
                       From (select 'X' empresa
                              ...
                              where f.codigoempresa || f.codigofl In (11,12,21,41,51,61,92,131,261,263,291)
                                and ff.competficha between ADD_MONTHS(Trunc(Sysdate,'rr'), -24) and last_day(sysdate) -- ADD_MONTHS(Trunc(Sysdate,'rr'), -96) and sysdate
                                and ff.codintfunc = f.codintfunc
                                and ff.situacaoffinan = 'A'
                                and (ff.tipofolha = 1)
