create or replace view pbi_dre_old as
Select v.previsto, v.realizado
     ...
     , (Select sum(r.PASSAGEIROS)
         ...
         Where r.EmpFil = v.EmpFil
           ...
           And to_char(r.Data,'yyyymm') = v.AnoMes) GratuidadeMesAtual
     , (Select sum(r.PASSAGEIROS)
          From pbi_arr_receita r
         Where r.EmpFil = v.EmpFil
           And r.VLR_FATURAMENTO <> 0
           And to_char(r.Data,'yyyymm') = v.AnoMes) PagantesMesAtual
...
Select  Sum(Decode(m.IdMetas,45, 1,-1) * v.previsto) previsto, Sum(Decode(m.IdMetas,45, 1,-1) * v.realizado) realizado
 ...
 Where v.Idmetas = m.Idmetas
   And v.idempresa = e.Idempresa
   And m.exibirnodre = 'S'
   And m.Idmetas In (45,2,22,23,95,42,43,50)
   And v.idempresa = d.Idempresa
   And v.referencia = d.referencia
   And d.fechado = 'S'
Group By
...
Select Sum(v.previsto) previsto, Sum(v.realizado) realizado
     ...
     , Substr(v.referencia,5,2) mes
     , To_number(substr(v.referencia,1,4)) Ano
     , 'Processos' Grupo, 'Administrativas' Agrupador, m.descricao Descricao
     , m.Idmetas, e.codigoglobus EmpFil
     , 24 ordem
     , d.datafechamento dataCorte
     , v.Idempresa
...
Select 0 previsto
 ...
 Group By To_number(AnoMes)
     ...
     , Sysdate
Union All
Select v.previsto  previsto
     , decode((Sum(TotalSos)+Sum(totalRa)),0,0, Sum(totalKm)/(Sum(TotalSos)+Sum(totalRa) ))   Realizado
     , To_number(To_Char(Data,'yyyymm')) AnoMes
     , To_Char(Data,'mm/yyyy') MEsAno
     , To_Char(Data,'mm') mes
     , To_number(To_Char(Data,'yyyy')) Ano
     , 'MKBF Modal' Grupo, 'MKBF Modal' Agrupador, 'Totalizador' Descricao
     , 0
     , EmpFil
     , 555 ordem
     , Data DataCorte
     , 0 IdEmpresa
     , 0 dissidio
  From Pbi_Mkbf m, niff_ads_valoresmetas v, niff_ctb_Dre d, niff_chm_empresas e
  Where (TotalSos > 0 Or totalRa > 0)
    And d.referencia = To_Char(Data,'yyyymm')
    And m.EmpFil = e. codigoglobus
    And e.idempresa = d.Idempresa
    And v.Idempresa = d.Idempresa
    And v.referencia = d.referencia
    And v.Idmetas = 6
    And To_number(To_Char(Data,'yyyy')) > 2017
    And (m.Modal Not Like ('VEICULO%') And m.Modal <> 'QUEIMADO')
 Group By To_number(To_Char(Data,'yyyymm'))
     , To_Char(Data,'mm/yyyy')
     , To_Char(Data,'mm')
     , To_number(To_Char(Data,'yyyy'))
     , Modal
     , EmpFil
     , Data
     , v.previsto
