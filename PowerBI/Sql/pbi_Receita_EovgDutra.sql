create or replace view pbi_arr_receita_eovgdutra as
Select -- 2017 e 2018
...
FROM
  ...
  (select Distinct t.valor, t.data From pbi_radar_operacional t where grupo = 'Dias uteis' And t.empfil = '001/001') d,
...
Select Distinct   '001/001 - EOVG DUTRA' empresa
     , '001/001' EmpFil
     , Data
     , 0 VLR_FATURAMENTO
     , 0 PASSAGEIROS
  ...
  From Niff_Calendario,
      (select t.Data dataPonte, t.empfil EmpFilPonte
        ...
        where grupo = 'Ponte de Feriado'
          And empfil = '001/001') t,
       FINFERIA_EMPRESAFILIAL F
 Where Data Not In (Select g.dat_prest_contas
                     From t_Arr_Guia g
                    Where g.dat_prest_contas Between ADD_MONTHS(Trunc(Sysdate,'rr'), -36) AND trunc(sysdate)-1
                      And Cod_empresa = 1
                      And CodigoFl = 1)
