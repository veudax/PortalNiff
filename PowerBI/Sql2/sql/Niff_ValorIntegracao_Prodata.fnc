        ...
        exception
          ...
          Begin
             Select trunc(    24*mod(pDataAtual - c.cu_datetime,1)) * 60 
              ...
              Where c.cuvp_perusectr = pbatida
                And c.cu_datetime Between pdataInicio And pdataInicio+1
                And c.cu_datetime = (Select Max(c1.cu_datetime)
                                       From CARDUSAGEVALIDPASSENGERS c1
                                      Where c1.cuvp_perusectr = pbatida
                                       And c1.cu_datetime Between pdataInicio And pdataInicio+1
                                       And c1.cu_datetime < pDataAtual
                                       And c1.ISS_ID ||'.'|| c1.CD_ID ||'.'|| c1.CRD_SNR = pCard)
                And c.ISS_ID ||'.'|| c.CD_ID ||'.'|| c.CRD_SNR = pCard;
            exception
              when no_data_found then
              tempoLimite := 0;
