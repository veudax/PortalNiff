-- SQL  BI --  OS OS
SELECT '001/001 - EOVG DUTRA' EMPRESA, to_char(B.LEVE) LEVE, TO_CHAR(C.PESADA) PESADA, A.*, 0 qtd
FROM

 (SELECT MOT.ABERTURA, to_char(MOT.OS) OS, DECODE(MOT.ORIGEM,2,'RA',3,'SOS')ORIGEM, MOT.CARRO, MOT.MOTORISTA, MEC.MECANICO, MOT.DEFEITO
  FROM
    ...
    (select DISTINCT m.dataaberturaos, m.numeroos, m.codorigos, f.codfunc||'-'||f.nomefunc MECANICO, v.prefixoveic
