...
Select Grupo, EmpFil, Funcionario, codfunc, chapafunc, nomefunc, ocorr, Data, pesoocorr, qtde, StatusFuncionario
  From (Select Codocorr, descocorr, 'Absenteísmo' grupo From frq_ocorrencia
         Where codocorr In (504,505,506,507,521,526,527,534,535,541,542,560,579,580,581,582,583,584,592,593,601,602)
         Union All
        Select Codocorr, descocorr, 'Outras' grupo From frq_ocorrencia
         Where codocorr Not In (504,505,506,507,521,526,527,534,535,541,542,560,579,580,581,582,583,584,592,593)) O,
       (Select LPad(fu.codigoempresa,3,'0') || '/' || Lpad(Decode(fu.codigoempresa, 9, decode(fu.codigofl, 2, 1, fu.codigofl), fu.codigofl),3,'0') EmpFil,
         ...
         --   and fu.situacaofunc = 'A'
           and f.codocorr in(2,3,28,84,85,86,89,209,221,222,233,236,241,263,264,292,294,299,314,344,502,506,507,514,519,521,525,530,534,535,538,545,546,549,559,560,583,584,592,593,594,597,601,602)
                                   ...
                                   From (select movto.empFil,
                                           ...
                                           From (select to_char(fu.codigoempresa,'000') || '/' || to_char(fu.codigofl,'000') EmpFil,
                                                  ...
                                                  Where t.dthist BETWEEN '01-jan-2017' AND ADD_MONTHS(LAST_DAY(trunc(Sysdate))+0, -1)
                                                    and f.codocorr = t.codocorr
         --                                            and fu.situacaofunc = 'A'
                                                    and f.codocorr in(2,3,28,84,85,86,89,209,221,222,233,236,241,263,264,292,294,299,314,344,502,506,507,514,519,525,530,534,538,559,560,583,545,535,521,546,549,584,592,593,594,597,601,602)
                                                    and t.codintfunc = fu.codintfunc
