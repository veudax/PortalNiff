...
--   and f.codocorr in(505,504,506,507,521,526,534,535,541,542,579,543,582,579,560,584,580,581,583,592,593,527,562)
    and f.codocorr In (504,505,506,507,521,526,527,534,535,541,542,560,579,580,581,582,583,584,592,593,601,602)
    and t.codintfunc = fu.codintfunc
    and fu.CODAREA in (20,30,40)
  group by t.dthist, fu.situacaofunc, fu.CODAREA)a,
(Select Total_func, Data, CodArea
   From (select Nvl(count(distinct f.nomefunc),0) total_func, Last_day(ff.competficha) data, f.codarea Codarea
          ...
          Union
         select Nvl(count(distinct f.nomefunc),0) total_func, Last_day(trunc(Sysdate)) data, f.codarea Codarea
           from vw_funcionarios f
          where f.codigoempresa = 1
            and f.codigofl = 1
            And f.SITUACAOFUNC = 'A'
         group by f.codarea)) B
...
UNION ALL
----------------------------------------------------------------------------
select '006/001 - VIAÇÃO ARUJÁ' empresa, Sum(a.qtd_ocorr) qtd_ocorr, a.dthist,  a.CODAREA, Max(total_func)
from
(select count(fu.nomefunc) qtd_ocorr, Last_day(trunc(t.dthist)) dthist, fu.situacaofunc, fu.CODAREA
from flp_historico t, frq_ocorrencia f, vw_funcionarios fu
where t.dthist BETWEEN '01-jan-2016' and sysdate
