...
and ac.codevento in (3,375,3,202,281,222,201,202,277,598,443,202,652,637,283)
    And f.CODINTFUNC =50
and ac.competacumu between '01-jan-2017' and (ADD_MONTHS(LAST_DAY(TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY'),'DD/MM/YYYY'))+0, -1))
  union all
 select '001/001 - EOVG DUTRA' Empresa
      , f.CODAREA
      , a.descarea Area
  ...
  where ac2.codintfunc = ac.codintfunc
    ...
    and fu.codfuncao in (1,170,241,242,327,393,413,502,526,527,65,671,674,699,76,773,99,2,226,228,432,700,79)
    and ac.codevento in (3,375,3,202,281,222,201,202,277,598,443,202,652,637,283)
    and ac.competacumu between '01-jan-2017'
    --(ADD_MONTHS(LAST_DAY(TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY'),'DD/MM/YYYY'))+1, -1)) 
    and (ADD_MONTHS(LAST_DAY(TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY'),'DD/MM/YYYY'))+20, -1))
    And f.CODINTFUNC =50)