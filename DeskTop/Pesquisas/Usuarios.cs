...
namespace Suportte.Pesquisas
{
    public partial class Usuarios : Form
    {
        ...
        private void confirmarButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                posIniId = value.IndexOf("UsuarioAcesso =") + 15;
                posFimId = value.IndexOf(", Tipo");
                Publicas._usuarioAcesso = value.Substring(posIniId, posFimId - posIniId).Trim();
            }
            catch { }
            finally
            {
        ...
        {
            mensagemSistemaLabel.Text = "Pesquisando usuários...";
            this.UseWaitCursor = true;
            this.Refresh();

            List<Classes.Usuario> _lista = new List<Usuario>();

            if (Publicas._idEmpresa != 0)
                _lista = new UsuarioBO().ListarUsuarios(true, Publicas._idEmpresa);
            else
                _lista = new UsuarioBO().ListarUsuarios(Publicas._apenasAtivos);
