    ...
    public partial class Jogos : Form
    {
        ...
        private void confirmarButton_Click(object sender, EventArgs e)
        {
            ...
            if (value == "" || value == null)
            {
                try
                {
                    value = gridGroupingControl.TableControl.Table.CurrentRecord.GetRecord().Info;
                }
                catch { }
            }
            ...
            try
            {
                posIniId = value.IndexOf("Data =") + 7;
                posFimId = value.IndexOf(", IdTime1");
                Publicas._codigoRetornoPesquisa = value.Substring(posIniId, posFimId - posIniId).Trim(); // retorna a Data
                Publicas._idRetornoPesquisa = 0;
                if (Publicas._dataPesquisa != DateTime.MinValue)
                {
                    posIniId = value.IndexOf("Id =") + 4;
                    posFimId = value.IndexOf(", Data");
                    Publicas._idRetornoPesquisa = Convert.ToInt32(value.Substring(posIniId, posFimId - posIniId).Trim()); // retorna o Id
        ...
        {
            List<Classes.BolaoJogos> _lista;

            if (Publicas._dataPesquisa == DateTime.MinValue)
                _lista = new BolaoJogosBO().Listar(DateTime.Now.Year);
            else
                _lista = new BolaoJogosBO().Listar(Publicas._dataPesquisa);

            Publicas._codigoRetornoPesquisa = string.Empty;
            Publicas._idRetornoPesquisa = 0;
            gridGroupingControl.DataSource = _lista.OrderBy(o => o.Data).ToList();
