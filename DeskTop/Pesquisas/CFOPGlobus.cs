...
{
    public partial class CFOPGlobus : Form
    {
        public CFOPGlobus() 
        {
            InitializeComponent();
        ...
        private void confirmarButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                posIniId = value.IndexOf("CFOP =") + 6;
                posFimId = value.IndexOf(", Descricao");
        ...
        private void CFOPGlobus_Shown(object sender, EventArgs e)
        {
            List<Classes.CFOPGlobus> _lista = new List<Classes.CFOPGlobus>();

            _lista = new CFOPGlobusBO().ListarCFOPSaidas();

            if (Publicas._cfopEntrada)
                _lista = new CFOPGlobusBO().ListarCFOPEntradas();
            else
            {
                if (Publicas._cfopSaida)
                    _lista = new CFOPGlobusBO().ListarCFOPSaidas();
                else
                    _lista = new CFOPGlobusBO().ListarCFOP();
            }

            Publicas._codigoRetornoPesquisa = string.Empty;
            Publicas._idRetornoPesquisa = 0;
            gridGroupingControl.DataSource = _lista.OrderBy(o => o.Descricao).ToList();
        ...
        private void CFOPGlobus_Load(object sender, EventArgs e)
        {
