...
namespace Suportte.Pesquisas
{
    public partial class Telas : Form
    {
        ...
        private void confirmarButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                posIniId = value.IndexOf("IdTela =") + 9;
                posFimId = value.IndexOf(", IdModulo");
                Publicas._idRetornoPesquisa = Convert.ToInt32(value.Substring(posIniId, posFimId - posIniId).Trim());
            }
            catch { }
            finally
            {
        ...
        {
            List<Tela> _lista = new TelaBO().Listar();
            Publicas._codigoRetornoPesquisa = string.Empty;
            gridGroupingControl.DataSource = _lista.OrderBy(o => o.IdTela).ToList();

            if (Publicas._idRetornoPesquisa != 0)
            {
                //Setting the filter condition.
                FilterCondition condition = new FilterCondition(FilterCompareOperator.Equals, Publicas._idRetornoPesquisa);
                //Setting the RecordFilterDescriptor
                RecordFilterDescriptor recordFilterDescriptor = new RecordFilterDescriptor("IdModulo", condition);
                //Adding the RecordFilterDescriptor to collection
                this.gridGroupingControl.TableDescriptor.RecordFilters.Add(recordFilterDescriptor);
            }
        ...
        private void gridGroupingControl_TableControlCurrentCellKeyDown(object sender, Syncfusion.Windows.Forms.Grid.Grouping.GridTableControlKeyEventArgs e)
        {
