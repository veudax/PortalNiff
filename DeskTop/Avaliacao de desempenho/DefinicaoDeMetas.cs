...
namespace Suportte.Avaliacao_de_desempenho
{
    public partial class DefinicaoDeMetas : Form
    {
                                    ...
                                    if (valor != 0)
                                    {
                                        switch (posicao)
                                        {
                                            case "1":
                                                ...
                                                item.Resultado1 = Math.Abs(Math.Round(item.Eficiencia1 != 0 ? ((item.Peso1 * item.Eficiencia1) / 100) : 0, 2));
                                                ...
                                                item.ResultadoPonderado1 = Math.Abs(Math.Round(item.EficienciaPonderada1 != 0 ? ((item.Peso1 * item.EficienciaPonderada1) / 100) : 0, 2));
        ...
        private void limparButton_Click(object sender, EventArgs e)
        {
            ...
            DataCorteLabel.Text = string.Empty;
            resultadoAvaliacaoLabel.Text = string.Empty;
            descricaoCargoTextBox.Text = string.Empty;
            referenciaMaskedEditBox.Text = string.Empty;
            referenciaFinalMaskedEditBox.Text = string.Empty;
        ...
        private void referenciaMaskedEditBox_Validating(object sender, CancelEventArgs e)
        {
            ...
            DateTime _dataIni = DateTime.MinValue;
            ...
            if (!ativoCheckBox.Checked) // se não estiver marcador para comparar referencias
            {
                ...
                try
                {
                    ...
                    _dataFim = Convert.ToDateTime("01/" + referenciaMaskedEditBox.Text).AddMonths(1).AddDays(-1);
                    _dataIni = Convert.ToDateTime("01/" + referenciaMaskedEditBox.Text);
                }
                catch
                {
                ...
                try
                {
                    _dataCorteFinanceiro = _listaItensAvaliacao.Max(m => m.DataCorteFinanceiro);
                    _dataCorteOperacional = _listaItensAvaliacao.Max(m => m.DataCorteOperacional);
                ...
                resultadoAvaliacaoLabel.Visible = _dataCorteOperacional != DateTime.MinValue && _dataCorteFinanceiro != DateTime.MinValue;
                    //(_autoAvaliacao.DataFim != DateTime.MinValue && _autoAvaliacao.Existe);

                resultadoAvaliacaoLabel.Text = "Resultado da Avaliação " + Math.Round(_listaItensAvaliacao.Sum(s => s.ResultadoPonderado) * _fatores.Fator, 2) .ToString();

                if (resultadoAvaliacaoLabel.Visible)
                {
                    _autoAvaliacao.TotalAvaliacao = Math.Round(_listaItensAvaliacao.Sum(s => s.ResultadoPonderado) * _fatores.Fator,2);

                    new AutoAvaliacaoBO().Gravar(_autoAvaliacao, null, _listaItensAvaliacao);
                }
            }
        }
                        ...
                        if (referencia.Month.ToString() + referencia.Year.ToString() == item.Referencia)
                        {
                            switch (i)
                            {
                                case 0:
                                    ...
                                    _mes.Resultado = Math.Abs(Math.Round(item.Resultado == 0 && _mes.Eficiencia != 0 ? ((item.Peso / _mes.Eficiencia)-1) * 100 : item.Resultado, 2));
                                    ...
                                    _mes.ResultadoPonderado = Math.Abs(Math.Round(_mes.EficienciaPonderada != 0 ? ((_mes.Peso * _mes.EficienciaPonderada) / 100) : 0, 2));
        ...
        private void gravarButton_Click(object sender, EventArgs e)
        {
            ...
            if (_listaItensAvaliacao.Where(w => w.EficienciaPonderada == 0).Count() == 0)
        ...
        private void DefinicaoDeMetas_Load(object sender, EventArgs e)
        {
        ...
        private void gridGroupingControl_QueryCellStyleInfo(object sender, GridTableCellStyleInfoEventArgs e)
        {
            ...
            { // buscar da empresa do usuario
                if (e.TableCellIdentity.RowIndex != -1 && (e.TableCellIdentity.Column.MappingName == "ValorEsperado" || e.TableCellIdentity.Column.MappingName == "Realizado"))
                {
                    ...
                    if (rec != null)
                    {
                        ...
                        if (dr != null && dr["Descricao"].ToString().Contains("Eficiência de Man"))
                        {
                            e.Style.Format = "N4";
                        }
                        else
                        if (dr != null && (dr["Descricao"].ToString().Contains("Retidos") || dr["Descricao"].ToString().Contains("MKBF") ||
                            dr["Descricao"].ToString().Contains("CNH") || dr["Descricao"].ToString().Contains("Exames") || dr["Descricao"].ToString().Contains("Reclamação")))
                        {
                            e.Style.Format = "N0";
                        }
                        else
